...
<img src="docs/assets/usap-banner.svg" alt="USAP banner — wordmark, tagline 'Agents reason · Humans approve · MCP executes', live stats (81 skills · 13 cs-* agents · 12 domains), and a hex constellation of representative agents around the central USAP hub" width="100%">
...
[![Skills](https://img.shields.io/badge/skills-81-29d3f0?style=flat-square&labelColor=05080d)](https://github.com/jaskaranhundal/usap-skills)
...
| Every skill emits a real payload | [`appsec-devsecops/vuln-scan/expected_outputs/sample_output.json`](appsec-devsecops/vuln-scan/expected_outputs/sample_output.json) (one of 81) |
...
| MITRE ATT&CK coverage doc | [`mappings/mitre-attack/coverage-summary.md`](mappings/mitre-attack/coverage-summary.md) — 80 distinct techniques cited across frontmatter and skill bodies; 18 of the 81 skills declare `mitre_attack` arrays in frontmatter (backfill is ongoing) |
...
| Security program with all 81 skills embedded | `dist/USAP_BUNDLE.md` | Claude, ChatGPT, Gemini |
...
|---|---|---|---|
| [`cs-security-analyst`](agents/security/cs-security-analyst.md) | Security | All 81 skills (full knowledge base) + 12 specialist agents | Universal security advisor — any question, any audience, any domain. Adapts to non-technical and expert users. Makes decisions. |
...
81 skill packages. 49 ship an executable analysis tool today; the other 32 `*_tool.py` scripts are declared stubs that exit non-zero with `status: not_implemented` and perform no analysis. The live census is in [`SELF-AUDIT.md`](SELF-AUDIT.md).
