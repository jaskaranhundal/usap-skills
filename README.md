...
<img src="docs/assets/usap-banner.svg" alt="USAP banner — wordmark, tagline 'Agents reason · Humans approve · MCP executes', live stats (80 skills · 13 cs-* agents · 12 domains), and a hex constellation of representative agents around the central USAP hub" width="100%">
...
[![Skills](https://img.shields.io/badge/skills-80-29d3f0?style=flat-square&labelColor=05080d)](https://github.com/jaskaranhundal/usap-skills)
...
The diagram above is **the** architecture diagram — a Signal-Architecture-styled hex constellation with the 12 orchestrator `cs-*` agents around the central USAP hub (`cs-usap-next`, the operator co-pilot, sits outside the ring). Domain colour-coding on the border: <kbd>security/</kbd> cyan · <kbd>appsec/</kbd> violet · <kbd>devsecops/</kbd> orange · <kbd>executive/</kbd> blue · <kbd>governance/</kbd> green. Regenerate with `python3 tools/gen_architecture_svg.py`.
...
```mermaid
flowchart LR
    SK["80 skills across<br/>12 domains<br/>(SKILL.md + scripts/)"]:::skill --> A
...
| Every skill emits a real payload | [`appsec-devsecops/vuln-scan/expected_outputs/sample_output.json`](appsec-devsecops/vuln-scan/expected_outputs/sample_output.json) (one of 80) |
...
| MITRE ATT&CK coverage doc | [`mappings/mitre-attack/coverage-summary.md`](mappings/mitre-attack/coverage-summary.md) — 80 distinct techniques cited across frontmatter and skill bodies; 18 of the 80 skills declare `mitre_attack` arrays in frontmatter (backfill is ongoing) |
...
| Security program with all 80 skills embedded | `dist/USAP_BUNDLE.md` | Claude, ChatGPT, Gemini |
...
- **Lite:** Alex answers directly from knowledge of all 80 USAP skills. Plain English by default; goes fully technical when you ask.
...
|---|---|---|---|
| [`cs-security-analyst`](agents/security/cs-security-analyst.md) | Security | All 80 skills (full knowledge base) + 12 specialist agents | Universal security advisor — any question, any audience, any domain. Adapts to non-technical and expert users. Makes decisions. |
...
| [`cs-usap-next`](agents/meta/cs-usap-next.md) | Meta | security-roadmap-planner | Operator co-pilot — reads runner, audit and registry state; recommends the next one to three actions |
...
80 skill packages. 12 ship an executable analysis tool today; the other 66 `*_tool.py` scripts are declared stubs that exit non-zero with `status: not_implemented` and perform no analysis. The live census is in [`SELF-AUDIT.md`](SELF-AUDIT.md).
...
python3 -m usap.cli validate-agents   # 80 skills + 13 cs-* agents valid
```
