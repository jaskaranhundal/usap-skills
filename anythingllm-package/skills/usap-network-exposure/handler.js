...
// USAP: Network Exposure
// Domain: detection
// Tool: detection/network-exposure/scripts/network-exposure_tool.py
...
module.exports.runtime = {
  handler: async function ({ input }) {
    try {
      ...
      const toolPath = path.join(repoPath, 'detection/network-exposure/scripts/network-exposure_tool.py');
      ...
      return result || JSON.stringify({ status: 'completed', skill: 'network-exposure', output: 'no output' });
    } catch (e) {
      return JSON.stringify({
        error: e.message,
        skill: 'network-exposure',
