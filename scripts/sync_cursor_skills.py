#!/usr/bin/env python3
"""Mirror the canonical SKILL.md tree into ``.cursor/skills/``.

Each ``<domain>/<slug>/SKILL.md`` is exposed as a top-level
``.cursor/skills/<slug>.md`` relative symlink so Cursor's agent rules
can reference USAP skills without forking content.
...
def main() -> int:
    ...
    changed, messages = sync_target(".cursor", clean=args.clean, check=args.check)
