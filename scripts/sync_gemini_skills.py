#!/usr/bin/env python3
"""Mirror the canonical SKILL.md tree into ``.gemini/skills/``.

Each ``<domain>/<slug>/SKILL.md`` is exposed as a top-level
``.gemini/skills/<slug>.md`` relative symlink so Gemini CLI can load
USAP skills without touching the canonical tree.
...
def main() -> int:
    ...
    changed, messages = sync_target(".gemini", clean=args.clean, check=args.check)
