...
- **De-stub risk-compliance tools** — all seven now ship real analysis tools, taking the repository to zero stub tools. `cyber-insurance` flags coverage gaps and denial-trigger exclusions; `enterprise-risk-assessment` tiers scenarios by FAIR Annual Loss Expectancy; `internal-audit-assurance` tests the control-plane audit dimensions and maps failures to ISO 27001 / SOC 2 / PCI-DSS / GDPR; `privacy-dpia` checks lawful basis, the Art. 35 DPIA requirement and data-subject rights; `quantum-security-readiness` tiers assets for post-quantum migration; `regulatory-horizon` flags applicable regulations the organization is not ready for; `risk-threat-modeling` classifies threats by STRIDE and scores them by likelihood × impact.
- **De-stub red-team tools** — all seven now ship real analysis tools. `ai-red-teaming` maps adversarial results to the MITRE ATLAS table and refuses unauthorized runs; `attack-path-analysis` scores paths by likelihood × impact and surfaces choke points; `continuous-pentesting` prioritises attack-surface changes P0/P1; `red-team-operations`, `red-team-planner` and `safe-exploitation` classify each requested action as read-only planning or an approval-gated execution directive (safe-exploitation additionally refuses any execution outside a lab environment); `security-research` scores emerging threats by evidence-based confidence and P0–P3 action priority.
- **De-stub platform-ai tools** — all seven now ship real analysis tools. `agent-integrity-monitor` classifies agent integrity violations (prompt injection, execution attempts, evidence tampering); `ai-agent-security` maps injection/misuse indicators to the read-only vs. mutating action table; `ai-ethics-governance` scores EU AI Act risk-tier gaps; `guardrail` enforces the control-plane checklist (approval, RBAC approver authority, separation of duties, blast-radius elevation) returning a `guardrail_result`; `orchestrator` routes events to the deterministic primary/secondary agent sequence; `third-party-vendor-risk` checks the tiered vendor evidence matrix; `tool-execution-broker` runs the eight pre-execution authorization checks and returns authorize or the first blocking reason.
- **De-stub appsec-devsecops tools** — all six now ship real analysis tools. `appsec-code-review` classifies findings against the OWASP Top 10 review table (severity + CWE); `build-integrity` scores SLSA gaps and the build-anomaly table and flags block-worthy artifacts (approval-gated); `devsecops-pipeline` applies the finding-severity table and the branch-aware block/warn/pass gate matrix (blocking is approval-gated); `sast-dast-coordinator` deduplicates across tools and prioritises DAST-confirmed exploitable findings; `secure-sdlc` scores the 0–5 maturity ladder and names the next gate; `supply-chain-simulation` computes detection coverage and flags missed critical detection points with MITRE techniques.
- **De-stub pentest tools** — `web-app-pentest` and `pentest-reporting` now ship real analysis tools. Web-app pentest maps findings to the OWASP Top 10 severity/MITRE table and refuses any target outside the supplied authorization scope (read-only classification; active exploitation stays an approval-gated L4 action). Pentest reporting compiles findings by the CVSS risk-rating and patch-SLA table (9.0+ critical / 24-hour emergency down to low / 90 days).
- **De-stub cloud-infra runtime tools** — `cloud-workload-protection`, `endpoint-os-security` and `ot-iot-device-security` now ship real analysis tools. Workload protection scores container-escape, C2 egress and reverse-shell signals against the CWPP coverage model; endpoint security scores the MITRE ATT&CK endpoint-indicator table (credential dumping, injection, persistence, LOLBins) and EDR blind spots; OT/IoT applies the OT paradox (availability and safety before patching) and gates every mutating OT action behind safety review plus CISO and operations-director approval.
- **`system-security/os-hardening` de-stubbed.** Classifies OS configuration findings against the SKILL.md CIS/STIG table with severity, intent and MITRE mapping (world-writable system files and unpatched local-privilege-escalation CVEs are critical) and prioritises remediation. Fixture, sample and manifest row added.
